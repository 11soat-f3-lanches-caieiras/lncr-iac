...
resource "aws_apigatewayv2_integration" "eks_nlb" {
  ...
  integration_uri        = "http://${data.eks-dns-nlb}"
...
resource "aws_apigatewayv2_authorizer" "lambda_integration" {
  ...
  authorizer_uri   = "arn:aws:apigateway:${var.defaul_region}:lambda:path/2015-03-31/functions/${var.lambda_function_arn}/invocations"
